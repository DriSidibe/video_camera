...
def capture():
    try:
        ...
        result = cv2.VideoWriter(
            f"{video_dir}{filename}.avi", cv2.VideoWriter_fourcc(*"MJPG"), 5, size
        )
        ...
        while True:
            ...
            if ret == True:
                ...
                # Press S on keyboard
                # to stop the process
                if cv2.waitKey(1) & 0xFF == ord("s") or datetime.now().hour == end_time:
                    break
...
while True:
    if launch_now or datetime.now().hour == start_time:
        launch_now = False
        capture()
